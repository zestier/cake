...
bx::FileReaderI* g_reader;
}

class RelativeReader : public bx::FileReader {
	const bx::FilePath root;

public:
	RelativeReader(const bx::FilePath& r) : root(r) {
	}

	virtual bool open(const bx::FilePath& _filePath, bx::Error* _err) override {
		bx::FilePath path = root;
		path.join(_filePath);
		return bx::FileReader::open(path, _err);
	}
};
...
class EmReader : public bx::FileReaderI {
	bx::MemoryReader* memReader = nullptr;
	void* mem = nullptr;

	virtual bool open(const bx::FilePath& _filePath, bx::Error* _err) override {
		int size = 0, err = 0;
		printf("Downloading %s\n", _filePath.getCPtr());
		(_filePath.getCPtr(), &mem, &size, &err);
		printf("Downloaded %s (%d bytes)\n", _filePath.getCPtr(), size);
		memReader = new bx::MemoryReader(mem, size);
		return err == 0;
	}
	...
	virtual int64_t seek(int64_t _offset, bx::Whence::Enum _whence) override {
		return memReader->seek(_offset, _whence);
	}

	virtual int32_t read(void* _data, int32_t _size, bx::Error* _err) override {
		return memReader->read(_data, _size, _err);
	}
};
#endif
...
void initBgfxUtils() {
	...
	g_reader = new RelativeReader("assets");
}
...
void* load(bx::FileReaderI* _reader, bx::AllocatorI* _allocator, const char* _filePath, uint32_t* _size)
{
	...
	{
		//SPDLOG_DEBUG("Failed to open: {0}.", _filePath);
	}
...
{
	return load(g_reader, g_allocator, _filePath, _size);
}
...
static const bgfx::Memory* loadMem(bx::FileReaderI* _reader, const char* _filePath)
{
	...
	//SPDLOG_DEBUG("Failed to load {0}.", _filePath);
	return NULL;
}
...
static void* loadMem(bx::FileReaderI* _reader, bx::AllocatorI* _allocator, const char* _filePath, uint32_t* _size)
{
	...
	//SPDLOG_DEBUG("Failed to load %s.", _filePath);
	return NULL;
}
...
{
	return loadShader(g_reader, _name);
}
...
{
	return loadProgram(g_reader, _vsName, _fsName);
}
...
{
	return loadTexture(g_reader, _name, _flags, _skip, _info, _orientation);
}

bimg::ImageContainer* imageLoad(const char* _filePath, bgfx::TextureFormat::Enum _dstFormat)
{
	uint32_t size = 0;
	void* data = loadMem(g_reader, g_allocator, _filePath, &size);
